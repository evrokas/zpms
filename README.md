...
Unlike DocArc, which commits flatpickr's built files directly into the
repo (`public/assets/vendor/flatpickr/` — a static asset needing no
install step), this app vendors the **actual flatpickr GitHub repo**,
gitignored, following the same "clone it yourself" convention already
used for `lib/ernsauth`/`lib/phpmailer` above. The one difference from
those: flatpickr's repo doesn't commit its own build output
(`dist/flatpickr.min.js`/`.css` are gitignored upstream, generated by
their own TypeScript toolchain), so getting a servable copy takes one
extra one-time build step, not just a checkout:

```sh
git clone https://github.com/flatpickr/flatpickr.git web/modules/flatpickr/vendor/flatpickr
cd web/modules/flatpickr/vendor/flatpickr
npm install
npm run build:build   # writes dist/flatpickr.min.js / dist/flatpickr.min.css
cd -
rm -rf web/modules/flatpickr/vendor/flatpickr/node_modules   # not needed after the build
```

This is a one-time setup step on a machine with Node available, not a
per-request or per-deploy build — nothing in the running app ever invokes
npm; the `flatpickr` module (`web/modules/flatpickr/`, listed under
`config/settings.info.yaml`'s `modules:`, same "always registered"
mechanism as `datepicker` before it) just serves the two built files as
plain static CSS/JS, `@app`-macro-resolved to
`web/modules/flatpickr/vendor/flatpickr/dist/flatpickr.min.{css,js}`.
Until that clone+build has been done on a given checkout, `edit_patient`'s
date-of-birth field simply 404s on those two asset requests and falls
back to being a plain text input — no PHP error, since nothing server-side
depends on the vendored files existing.
